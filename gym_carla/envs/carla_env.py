...
import subprocess
import os
from Sources.utils import HiddenPrints
...
class CarlaEnv(gym.Env):
  ...
  def __init__(self, params):
    ...
    start_server = True
    if(start_server):
      os.environ["CARLA_ROOT"] = '/home/shashank/prefcrl'
      ...
      launch_command += ['-carla-world-port=' + str(params['port'])]
    ...
    self.camera_trans = carla.Transform(carla.Location(x=-5.0, y=0.0, z=3.0))
    ...
    # Turn of rendering to speed up simulation
    ...
    self.cur_cam_obs = None
    self.collision_sensor = None
    self.lidar_sensor = None
    self.camera_sensor = None

    self.steer_range = params['continuous_steer_range']
  ...
  def step(self, action):
    ...
    if acc > 0:
      ...
      throttle = 0
      brake = np.clip(-acc/2,0,1)

    #Rescale actions from -1,1 to the actual steering range
    steer = rescale_action(steer, [-1, 1], self.steer_range)
    ...
    return (obs, self._get_reward(), done, done, copy.deepcopy(info))
  ...
  def render(self, mode=None):
    assert self.cur_cam_obs is not None, 'No camera observation found, please reset the environment.'
    return self.cur_cam_obs 
  ...
  def _is_collision_free(self, collision_box):
    """Check if there's any collision within the given collision box."""
    # Create a collision sensor
    collision_sensor = self.world.spawn_actor(
        self.world.get_blueprint_library().find('sensor.other.collision'),
        carla.Transform(), attach_to=None
    )
    ...
    def _on_collision(event):
        ...
        # Check if any collisions occur within the collision box
        for contact_point in event.contact_points:
            if collision_box.contains(contact_point):
    ...
    # Return True if no collision detected, False otherwise
  ...
  def _try_spawn_random_vehicle_at(self, transform, number_of_wheels=[4]):
    ...
    """
    # Check if there's any collision at the desired spawn location
    collision_free = self._is_collision_free(transform)

    if not collision_free:
        return False  # Abort spawn if there's a collision

    blueprint = self._create_vehicle_bluepprint('vehicle.*', number_of_wheels=number_of_wheels)
    if blueprint is None:
        return False  # No suitable blueprint found
  ...
  def _get_reward(self):
    ...
    # distance from target waypoint
    r_dist = -self.routeplanner.dist_target_waypoint()

    # r = 200*r_collision + 1*lspeed_lon + 10*r_fast + 1*r_out + r_steer*5 + 0.2*r_lat - 0.1
    r = 200*r_collision + 1*lspeed_lon + 1*r_out +1*r_dist + r_steer*1 - 0.1
    # r = 1*lspeed_lon

    return r

  def _get_cost(self):
    """Calculate the step costs."""
    return 0.0
